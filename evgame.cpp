...
Evgame::Evgame(int nett,int gamet,int imitt,int synct,int step,int LearnAbilityRateType,int moreThanType,int neighbors)
{
	...
	char *nets,*games,*imits,*syncs,*rate,*moreType,*stepName,*neib; //存放存盘文件名
	switch (nett)
	{
	...
	sprintf_s(fhead,60,"%s%s%s%s%s%s",nets,games,imits,rate,stepName,neib);//写磁盘文件
...
int Evgame::evgm(int neighbor)//非移动模式下的演化博弈过程
{
	...
	//				if (x==2) {dg=1.0;dr=0.8;}
        		...
        		NodeAdj banet(nettype,neighbor);//网络构造
